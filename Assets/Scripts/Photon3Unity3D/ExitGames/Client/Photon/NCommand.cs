...
namespace ExitGames.Client.Photon
{
	internal class NCommand : IComparable<NCommand>
	{
		...
		public override string ToString()
		{
			if (commandType == 1)
			{
				return string.Format("NC({0}|{1} ack s#/time {2}/{3})", commandChannelID, commandType, ackReceivedReliableSequenceNumber, ackReceivedSentTime);
			}
			return string.Format("NC({0}|{1} r/u: {2}/{3} st/r#/rt:{4}/{5}/{6})", commandChannelID, commandType, reliableSequenceNumber, unreliableSequenceNumber, commandSentTime, commandSentCount, timeoutTime);
		}
	}
}
