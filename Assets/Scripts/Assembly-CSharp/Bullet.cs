...
public class Bullet : Photon.MonoBehaviour
{
	...
	private void getSpiral(Vector3 masterposition, Vector3 masterrotation)
	{
		...
		if (spiralcount < 5)
		{
			float num5 = Vector2.Distance(new Vector2(masterposition.x, masterposition.z), new Vector2(base.gameObject.transform.position.x, base.gameObject.transform.position.z));
			num = num5;
		}
		else
		{
		...
		else
		{
			...
			else if (phase == 1)
			{
				...
				for (; i < value; i++)
				{
					...
					lineRenderer.SetPosition(i, new Vector3(UnityEngine.Random.Range(0f - num4, num4), UnityEngine.Random.Range(0f - num4, num4), UnityEngine.Random.Range(0f - num4, num4)) + myRef.transform.position + vector * ((float)i / (float)value) - Vector3.up * num * 0.05f * f - vector3 * 0.001f * f * num);
				}
			...
			else if (phase == 2)
			{
				if (leviMode)
				{
					...
					for (int j = 0; (float)j <= (float)(spiralNodes.Count - 1) - (float)spiralcount * 0.5f; j++)
					{
						if (spiralcount < 5)
						{
							Vector3 position = (Vector3)spiralNodes[j] + vector4;
							float num5 = (float)(spiralNodes.Count - 1) - (float)spiralcount * 0.5f;
							position = new Vector3(position.x, position.y * ((num5 - (float)j) / num5) + base.gameObject.transform.position.y * ((float)j / num5), position.z);
			...
			else
			{
				...
				nodes.Add(new Vector3(base.gameObject.transform.position.x, base.gameObject.transform.position.y, base.gameObject.transform.position.z));
				Vector3 vector5 = myRef.transform.position - (Vector3)nodes[0];
				for (int k = 0; k <= nodes.Count - 1; k++)
				{
					lineRenderer.SetVertexCount(nodes.Count);
					lineRenderer.SetPosition(k, (Vector3)nodes[k] + vector5 * Mathf.Pow(0.5f, k));
				}
				killTime2 += Time.deltaTime;
				if (killTime2 > 0.8f)
				{
